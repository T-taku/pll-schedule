...
@bot.slash_command(guild_ids=[737154037937340506])
async def create(
    ctx,
    ...
    bot.cur.execute("INSERT INTO plan(name, msg, year, month, day, hour, minutes) VALUES(?,?,?,?,?,?,?)",
                    (plan_name, plan_msg, year, month, day, hour, minutes))
    bot.conn.commit()
    embed = discord.Embed(
...
async def edit(
    ...
    plan_msg: Option(str, "メッセージ(メンションも含めます。)"),
    time: Option(
        str, "通知時刻を設定できます。(例:12時00分や、22:00なども可能)※半角数字のみ、1などの単数は01と入力 / 入力しない場合は0:00に通知されます。", required=False) = None,
):
    """予定の通知時間やメッセージを変更します。"""
    if time is None:
        hour = 0
    ...
    bot.cur.execute("UPDATE plan SET name = ?, msg = ?, year = ?, month = ?, day = ?, hour = ?, minutes = ? WHERE name = ?",
                    (plan_name, plan_msg, year, month, day, hour, minutes, plan))
    bot.conn.commit()
    embed = discord.Embed(
...
@bot.slash_command(guild_ids=[737154037937340506], name="delete")
async def delete_(
    ctx,
    plan: Option(str, "予定を選択してください。", autocomplete=get_plan),
):
...
@bot.slash_command(guild_ids=[737154037937340506], name="list")
async def list_(ctx):
    """予定の一覧を表示します。"""
...
async def send_notice():
    ...
    try:
        for i in bot.cur.fetchall():
            ...
            coros.append(bot.get_channel(928487681804087346).send(
                embed=embed))  # メッセージを送信するコルーチンを生成して配列にぶち込む
            coros.append(bot.get_channel(928487681804087346).send(i[1]))
            # データくんは用済みなので消し消し
            bot.cur.execute(
